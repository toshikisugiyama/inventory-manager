...
const actions = {
  ...
  async showMaterialItem ({commit}: {commit: Function}, materialId: string) {
    const response = await axios.get(url + '/' + materialId)
    if (response.status !== OK) {
      commit('error/setCode', response.status, {root: true})
      return false
    }
    commit('setMaterialItem', response.data)
  },
  ...
  resetMaterialState ({commit}: {commit: Function}) {
    const defaultMaterialItem: MaterialItem = {
      name: '',
      supplier: '',
      unit: ''
    }
    commit('setMaterialItem', defaultMaterialItem)
    commit('setSelectedMaterialIds', [])
    commit('isNotSelectedAllMaterials')
  }
}
